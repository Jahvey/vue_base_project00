...
public class ConContractInfo {
	...
	public String saveConContractDetailInfo(HashMap<String, Object> map){
		...
		if(null!=conDetail){
			 ...
			 if("tb_con_cktsyh".equals(tableName)){ //1  17-出口托收押汇批复表
				...
				CsmTbConJkxyzPo csmTbConJkxyzPo = new CsmTbConJkxyzPo();
				 ...
				 try {
					 ...
					 if("".equals(csmTbConXmdkcnhPo.getApplyDetailId())||null==csmTbConXmdkcnhPo.getApplyDetailId()){
						String applyDetId= UUIDGenerator.getUUID();
				 ...
				 try {
					 ...
					 if("".equals(csmTbConKlyhcdhpPo.getApplyDetailId())||null==csmTbConKlyhcdhpPo.getApplyDetailId()){
						String applyDetId= UUIDGenerator.getUUID();
			...
			}else if ("tb_con_gjxyzkz".equals(tableName)) {//10 unkown table name
				 ...
				 try {
					 ...
					 if("".equals(csmTbConGjxyzkzPo.getApplyDetailId())||null==csmTbConGjxyzkzPo.getApplyDetailId()){
						String applyDetId= UUIDGenerator.getUUID();
				...
				CsmTbConLdzjPo csmTbConLdzjPo = new CsmTbConLdzjPo();
				 ...
				 try {
					 ...
					 if("".equals(csmTbConCkxyzyhPo.getApplyDetailId())||null==csmTbConCkxyzyhPo.getApplyDetailId()){
						String applyDetId= UUIDGenerator.getUUID();
			...
			}else if ("tb_con_gjxyzdbdk".equals(tableName)) {//24 30-国际信用证打包贷款合同
				...
				CsmTbConGjxyzdbdkPo csmTbConGjxyzdbdkPo = new CsmTbConGjxyzdbdkPo();
				 ...
				 try {
					 ...
					 if("".equals(csmTbConGjxyzdbdkPo.getApplyDetailId())||null==csmTbConGjxyzdbdkPo.getApplyDetailId()){
						String applyDetId= UUIDGenerator.getUUID();
				 ...
				 try {
					 ...
					 if("".equals(csmTbConJkxyzyhPo.getApplyDetailId())||null==csmTbConJkxyzyhPo.getApplyDetailId()){
						String applyDetId= UUIDGenerator.getUUID();
			...
			}else if ("tb_con_ckttfprz".equals(tableName)) {//26 TODO 表tb_con_ckttfprz不存在
	 ...
	 * 2.根据贷种查出要查询的实体名称，由于品种信息表存的都是申请表实体，
	 * 需要将名称转换成合同的，再查询合同明细
	 * @param contractId
	 * @param productType
	 * @return
...
//		entityName=entityName.replace("Apply", "");
		...
		//查询批复利率信息
		TbBizAmountLoanrateApprovePo apvrate = csmTbBizAmountLoanrateApproveMapper.queryOneCsmTbBizAmountLoanrateApprove(tbConContractInfo.getAmountDetailId());
		//查询批复明细
		
		TbBizAmountDetailApprovePo tbBizAmountDetailApprove = csmTbBizAmountDetailApproveMapper.queryOneTbBizAmountDetailApproveByAmountDetailId(tbConContractInfo.getAmountDetailId());
		//获取营业时间
		Date handlingDate = gitUtils.getBusiDate();
