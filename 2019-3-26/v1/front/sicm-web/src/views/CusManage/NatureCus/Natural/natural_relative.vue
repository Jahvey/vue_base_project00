          ...
          <el-col :span="12">
            <el-form-item label="客户类型：" prop="custType">
              <el-select v-model="dataForm.custType" placeholder="请选择客户类型" :disabled="thisView">
                <el-option label="自然人客户" value="02"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
  ...
  export default {
    data(){
      return {
        ...
        // 界面数据
        dataForm: {
          legalContry: '',iscout: '',partyName: '',custType: '02',
    ...
    methods: {
      ...
      doPageQuery(listQuery){
        let query =  Object.assign({},listQuery,{partyId:this.$route.query.rowData.partyId,cType:'person'});
        this.$api.cusManage_naturalCus.getNaturalRelativeList(query).then((res) => {
          ...
          if(res && res.data && res.data.code == 200){//返回数据成功
            ...
            this.$message({
              message: '查询关系人信息个人信息信息失败！',
              type: 'error'
            });
          }
          this.$store.dispatch('setListLoading', false);//切记查询成功后取消遮罩
        }).catch((res) => {
          this.$message({message: '初始查询关系人信息个人信息失败, 错误信息：' + res, type: 'error'})
          console.error('初始查询关系人信息个人信息失败, 错误信息：' + res)
        }).then(data!=null?data.callback:'')
      },
      // 显示新增界面
      doAdd: function () {
        this.dialogLable = '新增关系人信息个人信息';
