...
<!--        <el-col :span="12">
          <el-form-item label="货币代号" prop="hbdh" >
            <el-select :disabled="inputComponentDisable"  v-model="data.hbdh" placeholder="请选择" style="width:100%">
              <el-option v-for="(value,key) in currencyCd" :key="key" :label="value" :value="key"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="钞汇标志" prop="chbz" >
            <el-select :disabled="inputComponentDisable"  v-model="data.chbz" placeholder="请选择" style="width:100%">
              <el-option v-for="(value,key) in chbzCd" :key="key" :label="value" :value="key"></el-option>
            </el-select>
          </el-form-item>
        </el-col>-->
        <el-col :span="12">
...
<script>
  import { addConCashDeposit} from '@/api/contractsign'
  import enums from "@/utils/enums"
    export default {
      name: "AddDepositWhenContract",
      props:{
        info:{
          type:Object,
        },
      ...
      data(){
        return{
          ...
          data:{
            ...
            subcontractId:this.info.subcontractId,//担保合同id
            contractId:this.info.contractId,//贷款合同id
        ...
        comfirm(){
          this.$refs["validate"].validate((valid) => {
            if(valid){
              ...
              this.buttonText = "";
              this.isLoading = true;
              addConCashDeposit(this.data).then(response => {
