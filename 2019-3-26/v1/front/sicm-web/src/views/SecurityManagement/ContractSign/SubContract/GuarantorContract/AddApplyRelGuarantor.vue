...
<script>
  ...
  import { selSubGrtBzr,checkIfCommitSubBasicInfo,insertSubGrtRel2} from '@/api/contractsign'
    export default {
        name: "add-apply-rel-guarantor",
      props:{
        info:{
          type:Object,
        },
          ...
          pageDef: {
            ...
            tabDef: {
              ...
              tabCols: [
                ...
                {label: "申请担保金额(元)", prop: "SURETY_AMT",},
      ...
      methods:{
        doPageQuery(listQuery){
          ...
          selSubGrtBzr(listQuery).then(response => {
        ...
        choose(row){
          ...
          checkIfCommitSubBasicInfo(obj).then(response => {
            if(response.data.code == enums.stateCode.code.success){
              if(response.data.TbConSubcontractPo.ifTopSubcon=='1'){ //1.当为最高额是需要另作操作
                var obj3={};
                obj3.suretyId = row.SURETY_ID;
                obj3.flag='showDialog';
                this.$emit('backFlag',obj3)
              } else{//2.非最高额直接保存
                var saveObj={};
