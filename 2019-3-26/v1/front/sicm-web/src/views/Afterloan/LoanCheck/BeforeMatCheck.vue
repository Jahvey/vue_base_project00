...
<script>
 // import CscFormDialog3 from "@/components/CscFormDialog/CscFormDialog3"
  ...
  import {queryBeforeMatCheck} from '@/api/Afterloan'
  import BeforeMatCheckInfo from './BeforeMatCheckInfo'//基本信息
  export default {
    name: "beforeMatCheck",
    data() {
      return {
        ...
        currentView:"BeforeMatCheckInfo",
        data: [
          {
            label: '首次检查信息',
            id:'1',
    ...
    components: { CscSingleTable,BeforeMatCheckInfo},

    methods: {
      doPageQuery(listQuery) {
        this.queryBeforeMatCheck(listQuery)
      },

      queryBeforeMatCheck(listQuery) {
        queryBeforeMatCheck(listQuery).then(response => {
      ...
      create(row){
        ...
        this.paramToCheckInfo={
          ...
          partyNum:row.partyNum,//客户类型
          productType:row.productType,//证件类型
          summaryNum:row.summaryNum,//证件号码
          summaryAmt:row.summaryAmt,//借据金额
          jjye:row.jjye,//借据余额
          beginDate:row.beginDate,//借据起期
          endDate:row.endDate,//借据止期
          isRisk:row.isRisk,//贷款用途
        }
      ...
      handleNodeClick(item,node,self) {//2018-12-25  点击选中
        if(item.id=='1-1'){
          //检查信息
          console.log("检查信息");
          this.currentView='BeforeMatCheckInfo'
        } else if(item.id=='2'){
          //影像资料
          console.log("影像资料");
        } else if(item.id=='3'){
