          ...
          link: function($S, element, attr, ctrl) {
            ...
            _NGUeditor = (function() {
              ...
              _NGUeditor.prototype.initEditor = function() {
                var _UMConfig, _editorId, _self;
                _self = this;
                if (typeof UM === 'undefined') {
                  console.error("Please import the local resources of ueditor!");
                  return;
                }
				if(typeof _self.modelContent === 'undefined'){
					_self.modelContent="";
				}
                _UMConfig = $S.config ? $S.config : window.UMEDITOR_CONFIG;
                _editorId = attr.id ? attr.id : "_editor" + (Date.now());
                element[0].id = _editorId;
                this.editor = UM.getEditor(_editorId,_UMConfig);
