(function($) {
	$.fn.extend({
		...
		diyUpload: function(opt, serverCallBack) {
			...
			//debugger;
			var webUploader = WebUploader.create(opt);
			...
			webUploader.on('fileQueued', function(file) {
				webUploader.createBox($fileInput, file);
			});
			//进度条事件
			...
			webUploader.on('uploadSuccess', function(file, response) {
				...
				if (successCallBack) {
					successCallBack.call($fileInput, response);
				}
			});
			//上传失败后触发事件;
			...
			webUploader.createBox = function($fileInput, file) {
				var file_id = file.id;
				var tipName = file.name || "请上传图片";
				...
				//创建按钮
				if (!file.isNotFile && $parentFileBox.find('.diyButton').length <= 0) {

					var div = '<div class="diyButton"> \
								<a class="diyStart" href="javascript:void(0)">开始上传</a> \
								<a class="diyCancelAll" href="javascript:void(0)">' + opt.cancelButtonText + '</a> \
							</div>';
				...
				//添加子容器;
				var cancelDom = file.isNotFile ? "" : '<div class="diyCancel"></div>';
				var li = '<li id="fileBox_' + file_id + '" class="diyUploadHover"> \
							<div class="viewThumb"></div> \
							<div class="diySuccess"></div> ' + cancelDom + ' \
							<div class="diyFileName">' + tipName + '</div>\
							<div class="diyBar"> \
				...
				if (opt.oneThumb) {
					$parentFileBox.children('.fileBoxUl').empty();
				};
				$parentFileBox.children('.fileBoxUl').append(li);
				//父容器宽度;
				var $width = $('.fileBoxUl>li').length * 450;
				...
				//生成预览缩略图
				if (file.isNotFile) {
					file.url = file.src || file.url;
					webUploader.makeViewThumb($fileBox, file.url);
				} else {
			...
			webUploader.makeViewThumb = function($fileBox, dataSrc) {
				if (!dataSrc) return;
				$fileBox.find('.viewThumb').append('<img src="' + dataSrc + '" >');
			};
			return webUploader;
	...
	function getOption(objId) {
		...
		return {
			...
			swf: "/js/fileupload/js/Uploader.swf"
		}
	};
	//操作进度条;
	...
	function removeLi($li, file_id, webUploader) {
		...
		if ($li.siblings('li').length <= 0) {
			...
			$li.parents('.parentFileBox').find('li').children().remove();
			$li.parents('.parentFileBox').find('.diyButton').remove();
            $li.remove();
		} else {
			$li.remove();
		}
