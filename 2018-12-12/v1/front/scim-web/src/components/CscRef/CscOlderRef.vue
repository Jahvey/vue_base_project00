<template>
  ...
  :displayFiledName="displayFiledName" :disabled="disabled" :entity="entity" @getRefValue="getRefValue" title="长者信息">
...
<script>
  ...
  export default {
      ...
      return {
        ...
        pageDef: {
          queryDef: {
            columnNum: 3, //一行几列
            queryCols: [
              {label: "姓名", inputType: "input", modelName: "olderName"},
          ...
          tabDef: {
            //表格字段定义
            tabCols: [
              {label: "姓名", prop:"olderName", isSort: true},
    ...
    methods: {
      doPageQuery(params){
        getOlderList(params).then(response => {
