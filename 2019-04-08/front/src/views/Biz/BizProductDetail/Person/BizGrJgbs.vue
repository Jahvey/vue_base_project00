<template>
  <div class="natural-gr-jgbs">
    <el-row class="warp">
      <el-col :span="24">
          <el-form ref="dashboard" :model="bizJG" label-width="140px">
            <el-col :span="12">
              <el-form-item label="客户分类" prop="cusType">
                <el-input v-model="bizJG.cusType"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="购房套数" prop="partyName">
                <el-input v-model="bizJG.houseSum"></el-input>
              </el-form-item>
            </el-col>
          </el-form>
      </el-col>
    ...
    <div v-if="buttonView">
      <el-button :size="size" type="primary" @click.native="saveAndUpdateYesOrNoApply" :loading="editLoading">保存</el-button>
    </div>
  </div>
...
<script>
  ...
  import { saveAndUpdateYesOrNoApply } from '@/api/csm'
  export default {
    name: "natural-gr-jgbs",
    props:{
      bizJG:Object,
    },
    data(){
        ...
        this.$refs['dashboard'].validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              let params = Object.assign({}, this.bizJG )
              console.log(params);
              saveAndUpdateYesOrNoApply(params).then((res) => {
