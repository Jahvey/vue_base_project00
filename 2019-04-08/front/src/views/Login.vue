<template>
  <el-form :model="loginForm" :rules="fieldRules" ref="loginForm" label-position="left" label-width="0px" class="demo-ruleForm login-container">
    ...
    <el-form-item style="width:100%;">
      ...
      <el-button type="primary" style="width:48%;" @click.native.prevent="login" :loading="loading">登1 录</el-button>
    </el-form-item>
  </el-form>
</template>
    ...
    login() {
      ...
      this.$api.login.login(userInfo).then((res) => {
          if(res && res.data && res.data.code != 200) {
            ...
            sessionStorage.setItem('userInfo',JSON.stringify(res.data.data.userInfo)) // 保存用户到本地会话
            sessionStorage.setItem('roles',JSON.stringify(res.data.data.roles)) // 保存用户到本地会话            this.$store.commit('menuRouteLoaded', false) // 要求重新加载导航菜单
            sessionStorage.setItem("currRole",res.data.data.roles[0].roleid)//默认角色第一个
