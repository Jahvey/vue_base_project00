...
<script>
  import { getSubCon,addMaxloancon} from '@/api/contractsign'
      ...
      beforeMount(){
        ...
        getSubCon(obj).then(response=>{
          if(response.data.code == enums.stateCode.code.success){//返回数据成功
              this.data.aviAmt = response.data.subcontractPo.aviAmt.toString();
          } else{
            this.$message({
              message: '获取数据失败啦！',
              type: 'error'
        ...
        confirm(){
          this.$refs["validate"].validate((valid) => {
            if(valid){
              ...
              var saveData={};
              saveData.subcontractNum=this.info.subcontractNum;
              saveData.contractId=this.info.contractId;
              saveData.subcontractId=this.info.subcontractId;
              saveData.suretyAmt=this.data.suretyAmt;
              saveData.subcontractType=this.info.subcontractType;
              addMaxloancon(saveData).then(response=>{
