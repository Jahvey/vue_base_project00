<template>
  <el-container class="edit-guarantor-contract-basic-info" v-loading.fullscreen.lock="isLoading">
        ...
        <el-form el-form label-width="200px"  size="mini" :model="data" label-position="right" :rules="rules" ref="validate" >
          <el-row>
            ...
            <el-col :span="9" :offset="3">
              <el-form-item label="保证合同编号" prop="subcontractNum">
                <el-input :disabled="inputComponentDisable || inputComponentDisable2" v-model="data.subcontractNum"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="9">
              <el-form-item label="保证合同纸质编号" prop="paperConNum">
                <el-input :disabled="inputComponentDisable" v-model="data.paperConNum"></el-input>
              </el-form-item>
            </el-col>
<!--            <el-col :span="9" :offset="3">
              <el-form-item label="保证人名称" prop="partyName">
                <el-input :disabled="inputComponentDisable || inputComponentDisable2" v-model="data.partyName"></el-input>
              </el-form-item>
            </el-col>-->
            <el-col :span="9" offset="3">
              <el-form-item label="是否最高额保证" prop="ifTopSubcon" >
                <el-select :disabled="inputComponentDisable || inputComponentDisable3"  @change="onValueChange" v-model="data.ifTopSubcon" placeholder="请选择" style="width:100%">
            ...
            <template v-if="data.ifTopSubcon =='0' || data.ifTopSubcon =='' || data.ifTopSubcon==null ">
              <el-col :span="9" offset="3">
                <el-form-item label="保证合同金额" prop="subcontractAmt">
                  <el-input :disabled="inputComponentDisable || inputComponentDisable4" v-model="data.subcontractAmt"></el-input>
                </el-form-item>
              </el-col>
            </template>
            <template v-if="data.ifTopSubcon =='1'">
              <el-col :span="9" offset="3">
                <el-form-item label="保证最高本金限额" prop="zgbjxe">
                  <el-input :disabled="inputComponentDisable" v-model="data.zgbjxe"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="9">
                <el-form-item label="保证额度起期" prop="beginDate" >
              ...
              <el-col :span="9" offset="3">
                <el-form-item label="保证额度止期" prop="endDate" >
            ...
            <el-col :span="9" offset="3">
              <el-form-item label="保证人股权变更达到（）%时，应取得债权人的书面同意" prop="changePercent">
                <el-input :disabled="inputComponentDisable" v-model="data.changePercent"></el-input>
              </el-form-item>
            </el-col>
            <template v-if="data.ifTopSubcon =='0' || data.ifTopSubcon =='' || data.ifTopSubcon==null ">
              <el-col :span="9" >
                <el-form-item label="保证债务百分比(%)" prop="dbRate">
                  <el-input :disabled="inputComponentDisable" v-model="data.dbRate"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="9" >
                <el-form-item label="保证范围" prop="subScope" >
              ...
              <el-col :span="21" :offset="3">保证范围说明：</el-col>
            ...
            <template v-if="dbxs">
              <el-col :span="9" :offset="3">
                <el-form-item label="保证保证方式" prop="proguarantyWay" >
                  <el-select :disabled="inputComponentDisable"  v-model="data.proguarantyWay" placeholder="请选择" style="width:100%">
                    <el-option v-for="(value,key) in proguarantyWayCd" :key="key" :label="value" :value="key"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="9" :offset="3">
                <el-form-item label="保证保证类型" prop="proguarantyType" >
                  <el-select :disabled="inputComponentDisable"  v-model="data.proguarantyType" placeholder="请选择" style="width:100%">
                    <el-option v-for="(value,key) in proguarantyTypeCd" :key="key" :label="value" :value="key"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="9" :offset="3">
                <el-form-item label="保证保证形式" prop="proguarantyForm" >
                  <el-select :disabled="inputComponentDisable"  v-model="data.proguarantyForm" placeholder="请选择" style="width:100%">
                    <el-option v-for="(value,key) in proguarantyFormCd" :key="key" :label="value" :value="key"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
            </template>
...
<script>
  ...
  import signMaxLoanConfirm from '../Common/SignMaxLoanConfirm'
    export default {
        name: "edit-guarantor-contract-basic-info",
      props:{
        info:{
          type:Object,
        },
      ...
      beforeMount(){
        ...
        getGrtConInfo(this.info).then(response=>{
          if(response.data.code == enums.stateCode.code.success){//返回数据成功
            if(this.info.contractType=="01"){
              this.data.ifTopSubcon='1';
              this.inputComponentDisable3=true;
            }
            if(response.data.contractBean.guarantyType.indexOf("04") != -1){
              this.dbxs = true;
            } else{
              this.dbxs = false;
            }
            this.rmb = response.data.contractBean.contractAmt;
            if(this.rmb!=null && this.rmb!=''){
              this.data.subcontractAmt = this.rmb;
            }
            if(response.data.contractBean.ifTopSubcon!=null && response.data.contractBean.ifTopSubcon!='' && response.data.contractBean.ifTopSubcon=='0'){//不是最高额
              if(this.rmb==null || this.rmb==''){
                this.$message({
                  message: '主合同基本信息未保存！',
                  type: 'error'
            ...
            console.log("数据："+JSON.stringify(response.data.contractBean));
            this.buttonText='确认';
      ...
      data(){
          return{
            ...
            dbxs:false,//保证方式，类型，形式是否显示
            rmb:"",//合同金额
            data:{
              subcontractNum:"",//保证合同编号
              //partyName:"",//保证人名称
              paperConNum:"",//保证合同纸质编号
              ifTopSubcon:"",//是否最高额保证
              bz:"",//币种
              subcontractAmt:"",//保证合同金额
              zgbjxe:"",//保证最高本金限额
              beginDate:"",//保证额度起期
              endDate:"",//保证额度止期
              signDate:"",//签订日期
              changePercent:"",//保证人股权变更达到（）%时，应取得债权人的书面同意
              subScope:"",//保证范围
              dbRate:"",//保证债务百分比(%)
              /*当主合同有保证时显示*/
              proguarantyWay:"",//保证保证方式
              proguarantyType:"",//保证保证类型
              proguarantyForm:"",//保证保证形式
              /*通知和文书送达*/
              aSendAddr:"",
              aPostcode:"",
              aReceiver:"",
...
<style scoped>
  .edit-guarantor-contract-basic-info h3,.edit-guarantor-contract-basic-info h4{
    border-bottom: 1px solid #ccc;
  }
  .edit-guarantor-contract-basic-info h4{
    margin-left: 20px;
    margin-right: 20px;
  }
</style>
