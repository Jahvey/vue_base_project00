package com.cdgit.loan.ledger.pubquery.resultbean;
/**固定查询*/
public class FixedQueryResult {
	private String orgNum;
	private String orgName;
	private String orgSeq;
	private String zchs;
	private String zcbs;
	private String zcye;
	private String gzhs;
	private String gzbs;
	private String gzye;
	private String cjhs;
	private String cjbs;
	private String cjye;
	private String kyhs;
	private String kybs;
	private String kyye;
	private String sshs;
	private String ssbs;
	private String ssye;
	private String hjhs;
	private String hjbs;
	private String hjye;
	private String hjdk;
	private String ssdk;
	private String kydk;
	private String cjdk;
	private String gzdk;
	public String getHjdk() {
		return hjdk;
	}
	public void setHjdk(String hjdk) {
		this.hjdk = hjdk;
	}
	public String getSsdk() {
		return ssdk;
	}
	public void setSsdk(String ssdk) {
		this.ssdk = ssdk;
	}
	public String getKydk() {
		return kydk;
	}
	public void setKydk(String kydk) {
		this.kydk = kydk;
	}
	public String getCjdk() {
		return cjdk;
	}
	public void setCjdk(String cjdk) {
		this.cjdk = cjdk;
	}
	public String getGzdk() {
		return gzdk;
	}
	public void setGzdk(String gzdk) {
		this.gzdk = gzdk;
	}
	public String getOrgNum() {
		return orgNum;
	}
	public void setOrgNum(String orgNum) {
		this.orgNum = orgNum;
	}
	public String getOrgName() {
		return orgName;
	}
	public void setOrgName(String orgName) {
		this.orgName = orgName;
	}
	public String getOrgSeq() {
		return orgSeq;
	}
	public void setOrgSeq(String orgSeq) {
		this.orgSeq = orgSeq;
	}
	public String getZchs() {
		return zchs;
	}
	public void setZchs(String zchs) {
		this.zchs = zchs;
	}
	public String getZcbs() {
		return zcbs;
	}
	public void setZcbs(String zcbs) {
		this.zcbs = zcbs;
	}
	public String getZcye() {
		return zcye;
	}
	public void setZcye(String zcye) {
		this.zcye = zcye;
	}
	public String getGzhs() {
		return gzhs;
	}
	public void setGzhs(String gzhs) {
		this.gzhs = gzhs;
	}
	public String getGzbs() {
		return gzbs;
	}
	public void setGzbs(String gzbs) {
		this.gzbs = gzbs;
	}
	public String getGzye() {
		return gzye;
	}
	public void setGzye(String gzye) {
		this.gzye = gzye;
	}
	public String getCjhs() {
		return cjhs;
	}
	public void setCjhs(String cjhs) {
		this.cjhs = cjhs;
	}
	public String getCjbs() {
		return cjbs;
	}
	public void setCjbs(String cjbs) {
		this.cjbs = cjbs;
	}
	public String getCjye() {
		return cjye;
	}
	public void setCjye(String cjye) {
		this.cjye = cjye;
	}
	public String getKyhs() {
		return kyhs;
	}
	public void setKyhs(String kyhs) {
		this.kyhs = kyhs;
	}
	public String getKybs() {
		return kybs;
	}
	public void setKybs(String kybs) {
		this.kybs = kybs;
	}
	public String getKyye() {
		return kyye;
	}
	public void setKyye(String kyye) {
		this.kyye = kyye;
	}
	public String getSshs() {
		return sshs;
	}
	public void setSshs(String sshs) {
		this.sshs = sshs;
	}
	public String getSsbs() {
		return ssbs;
	}
	public void setSsbs(String ssbs) {
		this.ssbs = ssbs;
	}
	public String getSsye() {
		return ssye;
	}
	public void setSsye(String ssye) {
		this.ssye = ssye;
	}
	public String getHjhs() {
		return hjhs;
	}
	public void setHjhs(String hjhs) {
		this.hjhs = hjhs;
	}
	public String getHjbs() {
		return hjbs;
	}
	public void setHjbs(String hjbs) {
		this.hjbs = hjbs;
	}
	public String getHjye() {
		return hjye;
	}
	public void setHjye(String hjye) {
		this.hjye = hjye;
	}
	
}
