...
<script>
  ...
  import {queryDailyCheckReport} from '@/api/afterloan'
  export default {
    name: "dailyCheckReport",
    data() {
      return {
        ...
        pageDef: {
          ...
          tabDef: {
            ...
            tabCols: [
              ...
              { label: '贷款金额', prop: 'amt', isSort: true },
    ...
    methods: {
      doPageQuery(listQuery) {
        this.queryDailyCheckReport(listQuery)
      },

      queryDailyCheckReport(listQuery) {
        queryDailyCheckReport(listQuery).then(response => {
