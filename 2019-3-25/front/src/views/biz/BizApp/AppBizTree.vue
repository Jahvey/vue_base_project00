        ...
        <el-container style="height: 540px; border: 1px solid #DCDFE6">
          ...
          <el-container>
            ...
            <el-main style="padding:10px;margin: 10px;margin-top:5px;margin-bottom:5px;border: 1px solid #E4E7ED;">
              <!--动态组件-->
                <compoment :is="currentView" :info="info" :bizInfo="bizInfo" :natural="natural"
                :tbBizXwApply="tbBizXwApply" :tbBizApply="tbBizApply" :amountDetail="amountDetail"
                :loanrate="loanrate" :XwDetail="XwDetail" :party="party" :bizJG="bizJG" :family="family"
                           :tbBizXmxxApply="tbBizXmxxApply" :tbCrdThirdPartyLimit="tbCrdThirdPartyLimit"></compoment>
            </el-main>
          </el-container>
        </el-container>
    </div>
...
<script>
  import AppBizInfoEdit from '../BizApp/AppBizInfoEdit'//基本信息
  import BizXwDetail from '../BizInfo/BizXwDetail'//业务明细
  import NaturalFamily from '../BizProductDetail/Person/NaturalFamily'//家庭财务信息
  import NaturalProject from '../BizProductDetail/Person/NaturalProject'//合作项目额度信息
  import BizTxList from '../BizProductDetail/Person/BizTxList'//贴息信息
  import BizGrJgbs from '../BizProductDetail/Person/BizGrJgbs'//监管报送信息
  ...
  import { getBizInfo} from '@/api/securitymanagement'
    export default {
      ...
      components:{
        AppBizInfoEdit, BizXwDetail, NaturalFamily, NaturalProject, BizGrJgbs, BizTxList,
        guarantor, deposit, creditInsurance, suggestion, mortgageGoods, pledgeGoods,
      },
      beforeMount(){
        //TODO 初始化数据，获取业务信息
      ...
      data() {
        return {
          ...
          party:Object,
          family:Object,
          tbCrdThirdPartyLimit:Object,
          tbBizXmxxApply:Object,
          bizJG:Object,
          data: [
            {
              label: '业务结构',
      ...
      methods:{
        ...
        handleNodeClick(item,node,self) {//2018-12-25  点击选中
          if(item.id=='1-1'){
            //基本信息
            this.currentView='AppBizInfoEdit';
          } else if(item.id=='1-2'){
            //业务明细
            this.currentView='BizXwDetail';
          } else if(item.id=='1-3'){
            //家庭财务信息
            this.currentView='NaturalFamily';
          } else if(item.id=='1-4'){
            //合作项目额度信息
            this.currentView='NaturalProject';
          } else if(item.id=='1-5'){
            //贴息信息
            this.currentView='BizTxList';
          } else if(item.id=='1-6'){
            //监管报送信息
            this.currentView='BizGrJgbs';
          } else if(item.id=='2-1'){
            //保证人
