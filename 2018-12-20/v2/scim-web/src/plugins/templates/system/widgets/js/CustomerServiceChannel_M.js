cb.widgets.register('CustomerServiceChannel', function (widgetType) {
    ...
    widget.prototype.runTemplate = function (error, result) {
        ...
        var isSupplier = this.isSupplier();
        if (isSupplier.isSupplier && result) {
            // 供应商电话
        ...
        self.ICCUrlFunc(newConfig, accountInfo, function (ICCUrl) {
            ...
            self.getElement().find('.floating_ckShop').html(html);
            if (newConfig.designer) {
                $$(".floating_ck").css("top", "50%");
            }
            if (newConfig.floatPosition == 'left') {
                self.getElement().find('.floating_ckShop').css('right', '');
                self.getElement().find('.floating_ckShop').css('left', '0px');
                self.getElement().find('.floating_ckShop dl dd').css('border-radius', '0 10px 10px 0');
            } else if (newConfig.floatPosition == 'right') {
                self.getElement().find('.floating_ckShop').css('left', '');
                self.getElement().find('.floating_ckShop').css('right', '0px');
                self.getElement().find('.floating_ckShop dl dd').css('border-radius', '10px 0 0 10px');
            }
            if (window.plus) {
                self.getElement().find('.plusPhoneBtnShop').on("click", function () {
        ...
        if (newConfig.serviceICC) {
            ...
            proxy.getContactWayUseInfo(function (serviceErr, serviceResult) {
                ...
                if (serviceResult.contacyType === "3" && serviceResult.youlianUrl) {
                    ...
                    var userData = "";
                    var exData = "show=" + encodeURIComponent(" ") + "&expandid1=" + encodeURIComponent(" ") + "&expandid2=" + encodeURIComponent(" ");
                    if (accountInfo) {
