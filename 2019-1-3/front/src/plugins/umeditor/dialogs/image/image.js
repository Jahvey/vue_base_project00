...
//                }
    ...
    var Upload = {
        ...
        uploadTpl: '<div class="edui-image-upload%%">' +
            ...
            '<input style=\"filter: alpha(opacity=0);\" multiple="multiple" class="edui-image-file" type="file" hidefocus name="upfile" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>' +
            '</form>' +
            '</div>',
        ...
        var params = utils.serializeParam(me.editor.queryCommandValue('serverparam')) || '',
          url = utils.formatUrl( url + (url.indexOf('?') == -1 ? '?':'&') + params);
        ...
        uploadComplete: function(r){
            ...
            try{
                ...
                if(r.indexOf("code")){
                    var resultMsg = r.substring(r.indexOf("{"), r.indexOf("}") + 1);
                    var resultObj = JSON.parse(resultMsg);
                    Base.callback(me.editor, me.dialog, '', resultObj['message']);
                }else{
                ...
                me.dialog.find('.edui-image-content').on('drop',function (e) {
                    ...
                    $.each(fileList, function (i, f) {
                        if (/^image/.test(f.type)) {
                            ...
                            var url=me.editor.getOpt('imageUrl') + "?type=ajax";
                            var params = utils.serializeParam(me.editor.queryCommandValue('serverparam')) || '',
                            url = utils.formatUrl( url + (url.indexOf('?') == -1 ? '?':'&') + params);
