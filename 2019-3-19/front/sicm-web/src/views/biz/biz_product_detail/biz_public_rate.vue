...
<template>
  <el-container>
    <el-main>
      ...
      <div id="tableloanrate" >
        ...
        <el-row :gutter="20" class="form">
          <el-form el-form label-width="200px" size="mini" :model="form2" ref="form2" label-position="right">
            <el-col :span="12">
              <el-form-item label="利率类型:" prop="loanOrg" class="special-input">
                <el-input v-model="form2.loanOrg" :disabled=false @blur="selectAccOrg"></el-input>
   
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="浮动方式:" v-show="isPayOutFlag" prop="payOutFlag"
                            class="special-input">
            ...
            <el-col :span="12">
              <el-form-item label="经办人:" prop="userNum" class="special-input">
                <el-input v-model="form2.userNum" :disabled=true></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="经办机构:" prop="payOutFlag" class="special-input">
                <el-input v-model="form2.orgNum" :disabled=true></el-input>
              </el-form-item>
            </el-col>
          </el-form>
        </el-row>
    ...
    <el-footer>
      <el-row :gutter="20">
        <el-col :span="6">
          <el-button type="primary" v-show="isSave" :disabled=!isSave @click="save" size="medium">保存</el-button>
        </el-col>
      </el-row>
    </el-footer>
  </el-container>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      let param = {loanId: '402881e967bb828101686fbaa3be0122', addr: 'pay/payout_info/loan_info'}
    ...
    methods: {
      initPayInfo(data) {
        getInitPayInfo(data).then(response => {//初始化pay_info的部分信息，流程id没有接上来
          ...
          let productType = o.contractInfo.productType
          let pjzl = o.loanInfo.pjzl
          let proFlag
          ...
          let flagType = o.bizHappenType//借新还旧标志
          let bizHappenType = flagType.substring(0, 2)
          let ynflag = flagType.substring(3, 4)
          if (bizHappenType != '06') {
          ...
          if ('1' != proFlag) {//如果流程标识为0表示为查看，隐藏保存按钮禁用控件，pay_apply已经赋了初始值	1
            this.isSave = false
            // this.isEnable()//无法实现
            // form.setEnabled(false);//整个表单（子组件和父组件表单）不可编辑
          }
          //如果开始时间或者结束时间存在，则计算基准利率
          let beginDate = o.loanInfo.beginDate
          let endDate = o.loanInfo.endDate
          ...
          //银承
          if ('01008001' == productType || '01008002' == productType || '01008010' == productType) {
            if (pjzl == "02") {//电票
              if (o.hps != null) {
      ...
      save() {//验证规则写在哪？form表单引入验证规则之后save需不需要也引入那些验证规则？？
        let childData = this.$refs.currentView.payChildForm//获取的子组件的form表单内容
